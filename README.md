...
- Users: `IsActive` soft-deactivation distinct from lockout (`SecurityUser` lockout), toggle Activate/Deactivate in list and detail; Role activation via `POST /api/roles/{id}/activate`; User password masked as `**********` in edit (optional, only sent when changed)
- Applications: confidential `ClientSecret` auto-generated (`RandomNumberGenerator`, 32 bytes base64) when empty and masked as `sk-****xyz` in list/detail; create shows `SecretAutoGenerated` hint
- Sessions page with search, date-range filters (`DateFrom`/`DateTo`) and pagination for users and session history; active sessions and history support filtering
- Breadcrumb navigation (`Components/Shared/Breadcrumb.razor`) on all Detail/Create pages (`Tenants > name`, `Roles > name`, etc.) and reusable `Pagination.razor` component
- Fixed full-viewport sidebar (`position: fixed; height: 100vh; overflow-y: auto`) with brand logo (`/img/logo.png` default from `assets/` or `Branding:LogoUrl`) and theme toggle; `brand-logo`/`auth-logo` CSS, `card-search`/`card-filters`/`pagination` layout
- Dashboard with live counts (users, connected users, roles, applications, scopes); `PersistentComponentState` in `MainLayout` preserves branding/theme/`isMasterAdmin` across prerender → WASM hydration and culture changes, eliminating the ~4s double-render flash
- Dark/light theme toggle plus toast (`IToastService`) and dialog (`IDialogService`) — toasts no longer lost on `forceLoad` navigations
...
│   │   │   ├── Endpoints/                        # /api/* minimal API groups, delegate to Services/ServerAdminXxxService (incl. BrandingEndpoints)
│   │   │   ├── Services/                         # ServerAdminXxxService: CQRS-backed IAdminXxxService implementations
│   │   │   ├── Options/                          # BrandingOptions mirror (actual lives in Shared/Options)
│   │   │   ├── wwwroot/img/                      # Default branding images (logo.png, full-logo.png from assets/)
...
│   │       ├── Interfaces/, Services/            # IAdminXxxService + HTTP-based AdminXxxService (QUERY via HttpMethod.Query)
│   │       ├── Models/                           # Client-facing DTOs + PagedRequest/PagedResponse<T>, PagedRequestWithDates
│   │       ├── Pages/, Components/               # Admin CRUD pages (search, pagination, breadcrumbs), Dashboard, Sessions (date range)
│   │       ├── Components/Shared/                # Breadcrumb.razor, Pagination.razor, ToastHost, DialogHost
│   │       └── Resources/                        # SharedResources.*.resx — localization (8 languages)
│   ├── Shared/
│   │   ├── OroIdentityServer.Shared/             # Shared contracts across host/client
│   │   └── Options/BrandingOptions.cs            # BrandingOptions (AppName/FullName/DisplayName/LogoUrl)
│   └── BuildingBlocks/
...
- **User `IsActive` soft-deactivation**: `User` aggregate now has `IsActive` + `Deactivate()`/`Activate()` + `UserDeactivatedEvent`/`UserActivatedEvent`, EF query filter `HasQueryFilter(u => u.IsActive)` and `IX_Users_IsActive`; `DeleteUser` now soft-deletes, new `POST /api/users/{id}/deactivate|activate` + `GetUserByIdIgnoringFilters` spec; login blocked when `!IsActive`
- **Role activation**: `Role.Activate()` + `RoleActivatedEvent` + `POST /api/roles/{id}/activate` via `GetRoleByIdIgnoringFilters`
- **Pagination & search (QUERY)**: `PagedRequest`/`PagedResponse<T>` + `PagedRequestWithDates`; `GetRoles/Users/Applications/Sessions` queries now filter server-side (`SearchTerm` contains) and paginate (`Skip`/`Take`, `TotalCount`); client services use `HttpMethod.Query` (`MapMethods(..., [HttpMethods.Query])` + `HttpRequestMessage(HttpMethod.Query, ...)`) for `/api/roles`, `/api/users/paged`, `/api/applications` (see `AdminUserSessionService.GetByUserAsync` pattern); `Pagination.razor` with page-size selector (10/20/50/100)
- **Applications secrets**: confidential `ClientSecret` masked as `sk-****xyz` in `MapApplication`, auto-generated (`RandomNumberGenerator.GetBytes(32)` base64) on create/update when empty, hint `SecretAutoGenerated` in UI
- **List pages UX**: `Roles/Users/Applications/Sessions` now have `card-search`/`card-filters` + date-range inputs (Sessions) and shared `Pagination`/`Breadcrumb`; `Users.razor` shows `Active/Inactive/Locked` badges, `Roles.razor` shows `Active` badge and activate button; detail pages show `IsActive` switch and persist via `IsActive` field
- **User edit password**: `UpdateUserRequest.Password: string?` (nullable), `UpdateUserCommand` + handler now only hashes when `Password != null && != "**********" && !empty` via `IPasswordHasher` + `SecurityUser.ChangePassword`; `UserDetail.razor` pre-fills `**********`, clears on focus, `Save` no longer requires password, allows deactivating without re-entry
- **Toasts & navigation**: removed `forceLoad:true` from `TenantCreate`/`TenantDetail` Activate/Suspend and `UserDetail.SyncRoles` — toasts now survive, pages reload via `LoadAsync`/`Get*ByIdAsync` instead of full reload
- **Navigation**: new `Breadcrumb.razor` + `BreadcrumbItem` on all Detail/Create pages (Tenants/Roles/Users/Applications/Scopes/IdentificationTypes) + CSS `.breadcrumb`; `IsActive` toggles added to `UserDetail`/`RoleDetail`
- **Layout**: sidebar fixed `position: fixed; height: 100vh; overflow-y: auto;` with `grid-column: 2` on `main`; `PersistentComponentState` (`layout` key) in `MainLayout` persists `isMasterAdmin`/`branding` across server prerender → WASM hydration and `/culture/set` changes (fixes ~4s flash)
- **Branding**: `Shared/Options/BrandingOptions` (`AppName`/`FullName`/`DisplayName`/`LogoUrl`/`FullLogoUrl` + `EffectiveLogoUrl` defaults `/img/logo.png`), registered via `Configure<BrandingOptions>(GetSection("Branding"))`, exposed at `GET /api/branding` (AllowAnonymous), consumed by `MainLayout` (via `IOptions` on server or `IAdminBrandingService` on WASM) and `Login` (via `IOptions`), CSS `brand-logo`/`auth-logo`; `DatabaseSeeder` and `OpenIddictServerConfiguration` now read `Branding:*` fallbacks; defaults use `assets/logo.png`/`FullNameImage.png` copied to `wwwroot/img/`
- **CSS**: `app.css` additions `.breadcrumb`, `.card-search`/`card-filters`, `.pagination`/`pagination-info`, `.brand-logo`/`.auth-logo`, sidebar fixed styles
...
- `src/IdentityServer/IdentityServer/appsettings.json` / `appsettings.Development.json` — logging, OpenIddict, DB connection, `Branding` (`AppName`/`FullName`/`DisplayName`/`LogoUrl`/`FullLogoUrl`)
- `src/Shared/Options/BrandingOptions.cs` — typed options with `EffectiveLogoUrl` defaults to `/img/logo.png` / `/img/full-logo.png` (copied from `assets/`)
...
| `Branding__AppName` | `OroIdentity` | Short brand name (sidebar, `MainLayout`) |
| `Branding__FullName` | `OroIdentityServer` | Full brand name (Login title, cert `CN`) |
| `Branding__DisplayName` | `OroIdentity Admin SPA` | OIDC client fallback (`DatabaseSeeder`) |
| `Branding__LogoUrl` | `/img/logo.png` | Sidebar/login logo URL (default `assets/logo.png`) |
| `Branding__FullLogoUrl` | `/img/full-logo.png` | Optional hero logo (default `assets/FullNameImage.png`) |
...
| Method | Route | Description |
|--------|-------|-------------|
| `GET` | `/api/users` | List all users (legacy, non-paged) |
| `QUERY` | `/api/users/paged` | Paged + search (`PagedRequest` JSON body: `SearchTerm`, `PageNumber`, `PageSize`) |
| `GET` | `/api/users/{id}` | Get user by ID |
| `POST` | `/api/users` | Create a new user |
| `PUT` | `/api/users/{id}` | Update a user (password nullable, `**********` masked — only changed when not masked) |
| `DELETE` | `/api/users/{id}` | Soft-delete (deactivate) a user — sets `IsActive=false` via query filter |
...
| Method | Route | Description |
|--------|-------|-------------|
| `QUERY` | `/api/roles` | Paged + search (`PagedRequest` via `QUERY` body) |
...
| Method | Route | Description |
|--------|-------|-------------|
| `QUERY` | `/api/applications` | Paged + search (confidential secrets auto-generated and masked `sk-****xyz`) |
| `GET` | `/api/applications/{clientId}` | Get application by client ID (secret masked) |
| `POST` | `/api/applications` | Create an OIDC application (auto-generates secret for confidential if empty) |
| `PUT` | `/api/applications/{clientId}` | Update an OIDC application |
| `DELETE` | `/api/applications/{clientId}` | Delete an OIDC application |

### Branding — `/api/branding` (AllowAnonymous)

| Method | Route | Description |
|--------|-------|-------------|
| `GET` | `/api/branding` | Returns effective `BrandingOptions` (used by `MainLayout` + `Login`) |
