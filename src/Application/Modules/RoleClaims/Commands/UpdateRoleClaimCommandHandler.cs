    ...
    ) : ICommandHandler<UpdateRoleClaimCommand>
{
    public async Task HandleAsync(UpdateRoleClaimCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var roleClaim = await roleRepository.GetRoleClaimByIdAsync(command.RoleClaimId, cancellationToken);
            ...
            await roleRepository.UpdateRoleClaimAsync(
                command.RoleClaimId, 
                new RoleClaim(
                new RoleClaimType(command.ClaimType), new RoleClaimValue(command.ClaimValue)), 
                cancellationToken);
