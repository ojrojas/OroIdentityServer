    ...
    public async Task<Result> HandleAsync(TerminateSessionCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            async Task RevokeTokenAsync(object token)
            {
                try
                {
                    var revokeMethod = tokenManager.GetType().GetMethod("RevokeAsync", [token.GetType(), typeof(CancellationToken)])
                        ?? tokenManager.GetType().GetMethod("RevokeAsync", [token.GetType()]);

                    if (revokeMethod != null)
                    {
                        var parms = revokeMethod.GetParameters();
                        var args = parms.Length == 2 ? new object[] { token, cancellationToken } : [token];
            ...
            if (!string.IsNullOrEmpty(session.AuthorizationId))
            {
                try
                {
                    ...
                    try
                    {
                        var findById = authorizationManager.GetType().GetMethod("FindByIdAsync", [typeof(string), typeof(CancellationToken)])
                            ?? authorizationManager.GetType().GetMethod("FindByIdAsync", [typeof(string)]);

                        if (findById != null)
                        {
                            var task = findById.Invoke(authorizationManager, findById.GetParameters().Length == 2 ? new object[] { session.AuthorizationId, cancellationToken } : [session.AuthorizationId]);
                            if (task is Task t)
                            {
                                await t.ConfigureAwait(false);
                    ...
                    if (authorization != null)
                    {
                        try
                        {
                            var revokeAuth = authorizationManager.GetType().GetMethod("RevokeAsync", [authorization.GetType(), typeof(CancellationToken)])
                                ?? authorizationManager.GetType().GetMethod("RevokeAsync", [authorization.GetType()]);

                            if (revokeAuth != null)
                            {
                                var args = revokeAuth.GetParameters().Length == 2 ? new object[] { authorization, cancellationToken } : [authorization];
                    ...
                    try
                    {
                        MethodInfo? findTokensMethod = tokenManager.GetType().GetMethod("FindByAuthorizationIdAsync", [typeof(string), typeof(CancellationToken)])
                            ?? tokenManager.GetType().GetMethod("FindByAuthorizationIdAsync", [typeof(string)]);

                        object? tokensEnumerable = null;
                        if (findTokensMethod != null)
                        {
                            tokensEnumerable = findTokensMethod.Invoke(tokenManager, findTokensMethod.GetParameters().Length == 2 ? new object[] { session.AuthorizationId, cancellationToken } : [session.AuthorizationId]);
                        }
                        ...
                        if (tokensEnumerable != null)
                        {
                            ...
                            {
                                var getAsyncEnumerator = tokensEnumerable.GetType().GetMethod("GetAsyncEnumerator", [typeof(CancellationToken)])
            ...
            try
            {
                ...
                try
                {
                    MethodInfo? findBySubjectMethod = authorizationManager.GetType().GetMethod("FindBySubjectAsync", [typeof(string), typeof(CancellationToken)])
                        ?? authorizationManager.GetType().GetMethod("FindBySubjectAsync", [typeof(string)]);

                    object? authsEnumerable = null;
                    if (findBySubjectMethod != null)
                    {
                        authsEnumerable = findBySubjectMethod.Invoke(authorizationManager, findBySubjectMethod.GetParameters().Length == 2 ? new object[] { subject, cancellationToken } : [subject]);
                    }
                    ...
                    if (authsEnumerable != null)
                    {
                        if (authsEnumerable is System.Collections.IEnumerable en2)
                        {
                            ...
                            {
                                var revokeMethod = authorizationManager.GetType().GetMethod("RevokeAsync", [auth.GetType(), typeof(CancellationToken)])
                                    ?? authorizationManager.GetType().GetMethod("RevokeAsync", [auth.GetType()]);

                                if (revokeMethod != null)
                                {
                                    var args = revokeMethod.GetParameters().Length == 2 ? new object[] { auth, cancellationToken } : [auth];
                        ...
                        {
                            var getAsyncEnumerator = authsEnumerable.GetType().GetMethod("GetAsyncEnumerator", [typeof(CancellationToken)])
                ...
                try
                {
                    MethodInfo? findTokensBySubjectMethod = tokenManager.GetType().GetMethod("FindBySubjectAsync", [typeof(string), typeof(CancellationToken)])
                        ?? tokenManager.GetType().GetMethod("FindBySubjectAsync", [typeof(string)]);

                    object? tokensBySubject = null;
                    if (findTokensBySubjectMethod != null)
                    {
                        tokensBySubject = findTokensBySubjectMethod.Invoke(tokenManager, findTokensBySubjectMethod.GetParameters().Length == 2 ? new object[] { subject, cancellationToken } : [subject]);
                    }
                    ...
                    if (tokensBySubject != null)
                    {
                        ...
                        {
                            var getAsyncEnumerator = tokensBySubject.GetType().GetMethod("GetAsyncEnumerator", [typeof(CancellationToken)])
